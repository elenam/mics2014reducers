...
%Readers familiar with these topics may wish to skip to section three.
...
If the value is used (by a function, for example), pmap will evaluate the entire expression immediately (i.e. eager evaluation), and always \emcomment{are you sure that it's always in parallel?} in parallel. A function \clocode{doall} in Clojure forces eager evaluation on lazy expressions. 

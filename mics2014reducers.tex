...
The test configurations are summarized in table~\ref{table:tests}. Here \clocode{map} and \clocode{reduce} are the standard serial functions, \clocode{pmap} is a parallel map as described in Section~\ref{sec:pmap}, and \clocode{r/fold} and \clocode{r/map} are reducers functions, as described in Section~\ref{sec:reducers}. We used the default settings for all of these functions. Note that because the r/fold configuration does not have a mapping phase, the test code for it needed to be rewritten slightly to move the functionality that was done by map in the other tests into the reducing stage. Section~\ref{sec:fermat} provides details of our testing code. 
...
The data used for this test is a collection of 100,000 random integers uniformly distributed between 0 and 1 billion. This test was run 100 times. 
...
In this section we show the results of running our tests across the machines mentioned above: in our tables, we often use red instead of reduce to achieve proper formatting.
...
\hline
Machine & reduce + map & reduce + pmap & r/fold + pmap & r/fold + map & r/fold & r/fold + r/map \\
\hline
...
\end{table}
 ...
 In the first four tests, \clocode{reduce + map, reduce + pmap, r/fold + map, r/fold + pmap}, almost all the combinations run in serial time, except for Box 1. Box has \clocode{reduce + pmap} and \clocode{r/fold + pmap} running in parallel time. \clocode{r/fold} and \clocode{r/fold + r/map} run on all boxes in parallel time.
...
\end{table}
 ...
 In the first four tests, \clocode{reduce + map, reduce + pmap, r/fold + map, r/fold + pmap}, almost all the combinations run in serial time, except for Box 1. Box has \clocode{reduce + pmap} and \clocode{r/fold + pmap} running in parallel time. \clocode{r/fold} runs on all boxes in parallel time.
...
The first three tests, execepting Box 2, run in serial time. Box 2 runs greater than serial time in \clocode{reduce + pmap} and \clocode{ r/fold + pmap} with \clocode{ reduce + map} running in serial time. \clocode{r/fold} and \clocode{r/fold + r/map} runs on all boxes in parallel time.
